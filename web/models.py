...
class TenantConfig(Base):
    ...
    # Guest web chat — unique token used in /guest/<token> URL
    # Auto-generated on first config creation. Lets hosts share a branded chat
...
# ===========================================================================
# Guest Web Chat — browser-based chat sessions for the /guest/<token> page
# ===========================================================================

class WebChatSession(Base):
    """
    One row per browser session on the guest web chat page.

    session_token   — UUID generated by the browser, stored in localStorage.
    reservation_id  — set once the guest identifies themselves with their
                      booking confirmation code.
    messages_json   — JSON list of {role, text, ts} — rolling conversation
                      history (last 30 messages kept).
    """
    ...
    reservation_id: Mapped[Optional[int]] = mapped_column(Integer, ForeignKey("reservations.id"), nullable=True, index=True)
    messages_json:  Mapped[Optional[str]] = mapped_column(Text, nullable=True)   # JSON [{role,text,ts}]
    created_at:     Mapped[datetime]      = mapped_column(DateTime(timezone=True), default=_now)
